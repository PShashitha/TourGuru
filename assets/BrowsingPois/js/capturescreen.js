...
var World = {
    ...
    loadPoisFromJsonData: function loadPoisFromJsonDataFn(poiData) {
        ...
        for (var currentPlaceNr = 0; currentPlaceNr < parseInt(poiData.length); currentPlaceNr++) {
            var open="";
            if(poiData[currentPlaceNr].opening_hours!==undefined){
                if(poiData[currentPlaceNr].opening_hours.open_now){
                    open = "►» Now Open";
                }else{
                    open = "►» Now Closed";
                }
            ...
            var userRating = 0;
            var placeName =  poiData[currentPlaceNr].name;
//            $.getJSON(wikiUrl,(data){
//                var desc = data.result.desc;
//            })
            userRating = poiData[currentPlaceNr].rating;
            var descriptionStr = ""+ open +"\n►» Has a total of "+userRating+" google user ratings.";
            var singlePoi = {
...
//            lon + "&" + ServerInformation.POIDATA_SERVER_ARG_NR_POIS + "=20";
        var serverUrl = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?"+
        "location=6.9130779,79.9724734&radius=10000&types=point_of_interest&key=AIzaSyBLcIpermU2uTd2ny81zbPVoWXNwQ8_6JU";
